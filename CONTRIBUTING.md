# 🧑‍💻 Contributing to `atomic-sre`

Welcome to atomic-sre! This document contains the instructions on how to contribute to atomic-sre.
...
Issues should be used to report bugs with `atomic-sre`, proposing new features before a PR is raised, and to request new features.
...
Pull Requests (PRs) are how contributions are made to `atomic-sre` and are always welcome.

The preferred way to contribute to `atomic-sre` is to fork the main repository on Github and then submit a pull request.

#### 🖥️ Getting setup locally

Here, we'll explain to get setup locally with `atomic-sre` and how to set up your git repository:

1. If you don't already have one, [create an account](https://github.com/join) on Github.

2. Fork the repository by clicking on the 'fork' button near the top of the page. What this will do is create a copy of the `atomic-sre` repository under your Github account. See [here](https://help.github.com/articles/fork-a-repo/) for a guide on how to fork a repository.

3. Clone this forked version of `atomic-sre` to your local disk:

```bash
git clone git@github.com:<your_username>/atomic-sre.git
cd atomic-sre
```
...
```bash
git remote add upstream git@github.com:DsThakurRawat/atomic-sre.git
```

5. Check that these remote aliases are correctly configured by running: `git remote -v`, this should show the following:

```bash
origin  git@github.com:<your_username>/atomic-sre.git (fetch)
origin  git@github.com:<your_username>/atomic-sre.git (push)
upstream        git@github.com:DsThakurRawat/atomic-sre.git (fetch)
upstream        git@github.com:DsThakurRawat/atomic-sre.git (push)
```

Now you have git properly configured, you can install the development dependencies which are described in [DEVELOPMENT.md](https://github.com/DsThakurRawat/Atomic-SRE/blob/main/DEVELOPMENT.md). Once the development environment is setup, you can start making changes by following these steps:
...
and you can now start making changes! We're sure that you don't need reminding but it's good practise to never work on the `main` branch and to always use a feature branch. The [DEVELOPMENT.md](https://github.com/DsThakurRawat/Atomic-SRE/blob/main/DEVELOPMENT.md) guide tells you how to run tests.
...
* Make sure your code passes the tests. At the moment, running the whole test suite doesn't take a long time so we advice doing that with `pytest` (see the [DEVELOPMENT.md](https://github.com/DsThakurRawat/Atomic-SRE/blob/main/DEVELOPMENT.md) guide).
