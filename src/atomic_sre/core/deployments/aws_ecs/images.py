...
def build_and_push_images(
    ...
    reporter(f"Building and pushing atomic-sre image ({TARGET_PLATFORM})")
    _run(
        [
            "docker",
            "build",
            ...
            TARGET_PLATFORM,
            "-t",
            f"{image_config.atomic_sre_uri}:{image_config.image_tag}",
            str(root_dir),
        ],
        reporter,
    )
    _run(
        ["docker", "push", f"{image_config.atomic_sre_uri}:{image_config.image_tag}"],
        reporter,
    )
