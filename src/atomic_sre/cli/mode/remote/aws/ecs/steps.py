...
from atomic_sre.cli.configuration.store import save_config
from atomic_sre.cli.env import load_env_values
from atomic_sre.cli.mode.paths import project_root
from atomic_sre.cli.mode.remote.aws.ecs.metadata import (
    default_iam_role_names,
    joined_secret_names,
    secret_arns,
)
from atomic_sre.cli.presentation.console import console
from atomic_sre.core.deployments.aws_ecs import (
    EcsDeploymentConfig,
    ImageBuildConfig,
    NetworkSelection,
    SecurityGroupInfo,
...
def ecs_config_from_cli(config: CliConfig) -> EcsDeploymentConfig:
    ...
    return EcsDeploymentConfig(
        ...
        ecr_repo_atomic_sre=config.ecs.ecr_repo_atomic_sre,
        ...
        ecr_atomic_sre_uri=config.deployment.ecr_atomic_sre_uri,
...
def run_security_group_step(config: CliConfig, ecs_config: EcsDeploymentConfig) -> CliConfig | None:
    ...
    description = "Security group for Atomic SRE ECS tasks"
...
def run_ecr_step(config: CliConfig, ecs_config: EcsDeploymentConfig) -> CliConfig | None:
    ...
    console.print("[dim]This stores the atomic-sre container image for ECS.[/dim]")
    session = create_session(ecs_config)

    report_step("Ensuring atomic-sre repository")
    atomic_sre_uri = ensure_repository(session, config.ecs.ecr_repo_atomic_sre)
    return _update_config_with_ecr(config, atomic_sre_uri)
...
def print_cleanup_summary(config: CliConfig) -> None:
    ...
    console.print(f"- ECR repo: {config.ecs.ecr_repo_atomic_sre}")
...
def print_deployment_summary(config: CliConfig) -> None:
    ...
    console.print(f"- Create ECR repository: {config.ecs.ecr_repo_atomic_sre}")
    console.print("- Build and push the atomic-sre container image")
...
def reset_cleanup_state(config: CliConfig) -> None:
    ...
    config.deployment.ecr_atomic_sre_uri = None
...
def _update_config_with_ecr(config: CliConfig, atomic_sre_uri: str) -> CliConfig:
    """Persist ECR repository URI to config.

    Args:
        config: CLI configuration values.
        atomic_sre_uri: SRE agent repository URI.

    Returns:
        The updated configuration.
    """
    config.deployment.ecr_atomic_sre_uri = atomic_sre_uri
