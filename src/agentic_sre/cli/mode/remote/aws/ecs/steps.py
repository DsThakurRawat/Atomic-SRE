...
from agentic_sre.cli.configuration.store import save_config
from agentic_sre.cli.env import load_env_values
from agentic_sre.cli.mode.paths import project_root
from agentic_sre.cli.mode.remote.aws.ecs.metadata import (
    default_iam_role_names,
    joined_secret_names,
    secret_arns,
)
from agentic_sre.cli.presentation.console import console
from agentic_sre.core.deployments.aws_ecs import (
    EcsDeploymentConfig,
    ImageBuildConfig,
    NetworkSelection,
    SecurityGroupInfo,
...
def ecs_config_from_cli(config: CliConfig) -> EcsDeploymentConfig:
    ...
    return EcsDeploymentConfig(
        ...
        ecr_repo_agentic_sre=config.ecs.ecr_repo_agentic_sre,
        ...
        ecr_agentic_sre_uri=config.deployment.ecr_agentic_sre_uri,
...
def run_security_group_step(config: CliConfig, ecs_config: EcsDeploymentConfig) -> CliConfig | None:
    ...
    description = "Security group for Agentic SRE ECS tasks"
...
def run_ecr_step(config: CliConfig, ecs_config: EcsDeploymentConfig) -> CliConfig | None:
    ...
    console.print("[dim]This stores the agentic-sre container image for ECS.[/dim]")
    session = create_session(ecs_config)

    report_step("Ensuring agentic-sre repository")
    agentic_sre_uri = ensure_repository(session, config.ecs.ecr_repo_agentic_sre)
    return _update_config_with_ecr(config, agentic_sre_uri)
...
def print_cleanup_summary(config: CliConfig) -> None:
    ...
    console.print(f"- ECR repo: {config.ecs.ecr_repo_agentic_sre}")
...
def print_deployment_summary(config: CliConfig) -> None:
    ...
    console.print(f"- Create ECR repository: {config.ecs.ecr_repo_agentic_sre}")
    console.print("- Build and push the agentic-sre container image")
...
def reset_cleanup_state(config: CliConfig) -> None:
    ...
    config.deployment.ecr_agentic_sre_uri = None
...
def _update_config_with_ecr(config: CliConfig, agentic_sre_uri: str) -> CliConfig:
    """Persist ECR repository URI to config.

    Args:
        config: CLI configuration values.
        agentic_sre_uri: SRE agent repository URI.

    Returns:
        The updated configuration.
    """
    config.deployment.ecr_agentic_sre_uri = agentic_sre_uri
