...
from atomic_sre.engine.orchestrator import _get_model, _infer_provider
from atomic_sre.engine.settings import AgentSettings, AWSSettings, GitHubSettings, SlackSettings
...
class TestGetModel:
    ...
    def test_anthropic_returns_chat_anthropic(self, _base_settings: AgentSettings) -> None:
        ...
        _base_settings.model = "anthropic:claude-sonnet-4-6"
    ...
    def test_ollama_returns_chat_openai_with_custom_base(
        self, _base_settings: AgentSettings
    ) -> None:
        ...
        model = _get_model(_base_settings)
        assert isinstance(model, ChatOpenAI)
        assert str(model.openai_api_base) == "http://custom-ollama:11434/v1"
    ...
    def test_bare_claude_auto_routes_to_anthropic(self, _base_settings: AgentSettings) -> None:
        ...
        _base_settings.model = "claude-sonnet-4-6"
        _base_settings.anthropic_api_key = "test-key"
        model = _get_model(_base_settings)
        assert isinstance(model, ChatAnthropic)

    def test_model_whitespace_handling(self, _base_settings: AgentSettings) -> None:
        """Accidental spaces in model name should be stripped correctly."""
        _base_settings.model = " anthropic : claude-sonnet-4-6 "
        _base_settings.anthropic_api_key = "test-key"
        model = _get_model(_base_settings)
        assert isinstance(model, ChatAnthropic)
        assert cast(Any, model).model == "claude-sonnet-4-6"

    def test_api_key_whitespace_stripping(self, _base_settings: AgentSettings) -> None:
        """Whitespace in API key should be stripped."""
        _base_settings.model = "anthropic:claude-sonnet-4-6"
        _base_settings.anthropic_api_key = "  test-key-with-spaces  \n"
        model = _get_model(_base_settings)
        assert isinstance(model, ChatAnthropic)
        assert cast(Any, model).anthropic_api_key.get_secret_value() == "test-key-with-spaces"
    ...
    def test_whitespace_only_api_key_raises_value_error(
        ...
        _base_settings.model = "anthropic:claude-sonnet-4-6"
        _base_settings.anthropic_api_key = "   "
        with pytest.raises(ValueError, match="ANTHROPIC_API_KEY"):
            _get_model(_base_settings)
